...
export default function ServiceDetails() {
  ...
  const [viewingImage, setViewingImage] = useState(null); // ✅ Para abrir y descargar
  ...
  // 3. FUNCIONES DE ENVÍO Y ARCHIVOS
  ...
  const handleDownload = async (url) => {
    try {
      ...
      link.href = blobUrl;
      link.download = `Referencia-${event.title}-${Date.now()}.jpg`;
        ...
        <div className="bg-white rounded-[35px] shadow-xl border border-slate-100 flex flex-col min-h-[550px] max-h-[700px] overflow-hidden">
          <div className="flex flex-col flex-shrink-0 sticky top-0 z-40 bg-white">
            ...
            {pinnedMessage && (
              <div className="bg-brand-600 text-white p-4 shadow-lg flex items-start gap-3 animate-slide-down">
                ...
                <div className="flex-1"><p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Mensaje Anclado</p><p className="text-sm font-bold leading-relaxed">{pinnedMessage.text}</p></div>
                {userRole === 'pastor' && <button onClick={() => togglePin(pinnedMessage.id, true)} className="p-1 hover:bg-white/10 rounded-full"><X size={16}/></button>}
              </div>
            )}
          </div>
            ...
            {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex flex-col ${isMyMessage ? 'items-end' : 'items-start'}`}>
                      <div className={`max-w-[90%] p-1 rounded-[22px] shadow-sm relative group ${isMyMessage ? 'bg-amber-600 text-white rounded-tr-none' : 'bg-slate-100 text-slate-800 rounded-tl-none border border-slate-200/40'}`}>
                          {/* ✅ IMAGEN CLICKEABLE */}
                          {m.image && (
                            <div className="relative group/img cursor-zoom-in" onClick={() => setViewingImage(m.image)}>
                              <img src={m.image} className="w-full h-auto rounded-[18px]" alt="Adjunto" />
                              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center rounded-[18px]">
                          ...
                          {m.text && <p className="font-semibold leading-relaxed whitespace-pre-wrap px-4 py-3 text-sm">{m.text}</p>}
                          <div className={`absolute top-0 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity ${isMyMessage ? '-left-16' : '-right-16'}`}>
                             {(userRole === 'pastor' || isMyMessage) && <button onClick={() => deleteMessage(m.id)} className="p-2 bg-red-50 text-red-500 rounded-full"><Trash2 size={14}/></button>}
                             {userRole === 'pastor' && !m.image && !m.checklist && <button onClick={() => togglePin(m.id, m.isPinned)} className="p-2 bg-brand-50 text-brand-600 rounded-full"><Pin size={14}/></button>}
                          </div>
                      </div>
          ...
          <div className="border-t border-slate-100 bg-slate-50/50 p-4">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-end gap-3">
              ...
              <button type="submit" disabled={isSending} className="bg-brand-600 text-white p-4 rounded-2xl shadow-xl active:scale-90 flex-shrink-0 disabled:opacity-50">{isSending ? <Loader2 size={22} className="animate-spin"/> : <Send size={22}/>}</button>
            </form>
          </div>
        </div>
      </div>

      {/* ✅ MODAL DE IMAGEN CON DESCARGA */}
      {viewingImage && (
        <div className="fixed inset-0 z-[150] bg-black/95 flex flex-col items-center justify-center p-4 animate-fade-in" onClick={() => setViewingImage(null)}>
          <div className="absolute top-8 right-6 flex gap-4">
            <button onClick={(e) => { e.stopPropagation(); handleDownload(viewingImage); }} className="p-3 bg-white/10 rounded-full text-white hover:bg-white/20"><Download size={24}/></button>
            <button onClick={() => setViewingImage(null)} className="p-3 bg-white/10 rounded-full text-white hover:bg-white/20"><X size={24}/></button>
          </div>
          <img src={viewingImage} className="max-w-full max-h-[80vh] object-contain rounded-xl shadow-2xl" onClick={e => e.stopPropagation()} />
          <p className="text-white/40 text-[10px] mt-6 font-bold uppercase tracking-widest">Toca fuera para cerrar</p>
        </div>
      )}
