...
const PostSkeleton = () => (
  <div className="bg-white p-4 rounded-2xl animate-pulse mb-4">
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white pb-32 font-sans relative">
      ...
      <div className="flex justify-between items-center px-4 pt-4 pb-2 bg-white sticky top-0 z-40">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">CDS App.</h1>
        <div className="flex items-center gap-2">
          {birthdays.length > 0 && (
            <button onClick={() => setIsBirthdayModalOpen(true)} className="relative w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors">
      ...
      {/* Stories Carousel */}
      <div className="flex gap-4 px-4 pb-4 pt-2 overflow-x-auto no-scrollbar border-b border-gray-100 bg-white">
        {canCreatePost && (
          <div className="flex flex-col items-center gap-1 min-w-fit cursor-pointer" onClick={() => { setEditingPost(null); setIsModalOpen(true); }}>
            <div className="w-16 h-16 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center active:scale-95 transition-transform">
      ...
      {/* Navigation Pills */}
      <div className="flex gap-2 px-4 py-4 overflow-x-auto no-scrollbar bg-white sticky top-[60px] z-30">
        ...
        {isPastor && (
          <button onClick={() => { setFilter('Archivados'); setVisibleCount(5); }} 
            className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${
              filter === 'Archivados' ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
            }`}>
            Archivados
          </button>
        )}
      ...
      <div className="px-4 space-y-6">
            ...
            displayedPosts.map(post => {
              ...
              <div key={post.id} className="pb-6 border-b border-gray-100 last:border-0">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {isModerator && (
                    <div className="relative">
                      <button onClick={(e) => { e.stopPropagation(); setMenuOpenId(menuOpenId === post.id ? null : post.id); }} className="p-2 text-gray-400 hover:text-gray-700 transition-colors">
                      ...
                      {menuOpenId === post.id && (
                        <div className="absolute right-0 top-8 bg-white shadow-xl rounded-xl border border-gray-100 py-2 w-48 z-50">
                ...
                <div className="cursor-pointer" onClick={() => navigate(`/post/${post.id}`)}>
                  {post.title && <h2 className="text-[15px] font-bold text-gray-900 mb-1">{post.title}</h2>}
                  <p className="text-[14px] text-gray-700 mb-3 leading-relaxed whitespace-pre-wrap line-clamp-4">
                    {post.content}
                  </p>
                </div>
                ...
                {/* Post Image */}
                {post.image && (
                  <div className="mb-3 cursor-pointer" onClick={() => navigate(`/post/${post.id}`)}>
                ...
                <div className="flex items-center justify-between py-1 relative">
                  <div className="flex items-center gap-5">
                    ...
                    <div className="relative flex items-center gap-1.5">
                      ...
                      <button onClick={() => setActiveReactionPost(activeReactionPost === post.id ? null : post.id)} className="p-1 active:scale-75 transition-transform">
                      ...
                      <span className="text-[13px] font-medium text-gray-500">{post.reactions?.length || 0}</span>
                    </div>

                    <div className="flex items-center gap-1.5" onClick={() => navigate(`/post/${post.id}`)}>
                      <button className="p-1 active:scale-75 transition-transform">
                        <svg className="w-[22px] h-[22px] text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                        </svg>
                      </button>
                      <span className="text-[13px] font-medium text-gray-500">{post.commentsCount || 0}</span>
                    </div>
                  </div>
                  
                  {/* Vista / Compartir Icono opcional a la derecha */}
        ...
        {hasMorePosts && !loading && (
          <div className="flex justify-center mt-2 pb-8">
            <button onClick={() => setVisibleCount(prev => prev + 5)} className="text-[13px] font-semibold text-blue-500 py-2 active:scale-95 transition-transform">
      ...
      <div className="fixed bottom-6 left-4 right-4 bg-white rounded-full shadow-2xl border border-gray-100 flex items-center justify-around py-3 px-2 z-40">
        ...
        <button onClick={() => { if(canCreatePost){ setEditingPost(null); setIsModalOpen(true); } }} className="w-14 h-14 bg-blue-500 rounded-full flex items-center justify-center shadow-lg shadow-blue-500/30 -mt-6 active:scale-95 transition-transform">
