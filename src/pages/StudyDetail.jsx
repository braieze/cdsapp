...
export default function StudyDetail() {
  ...
  const [selectedStudent, setSelectedStudent] = useState(null); // Para el Modal de Detalle
    ...
    <div className="pb-40 pt-4 bg-slate-50 min-h-screen animate-fade-in font-outfit text-left">
      ...
      {viewMode === 'lessons' ? (
        <div className="px-6 space-y-4 animate-slide-up">
          ...
          {lessons.map((lesson, index) => {
            const isCompleted = studentsProgress.find(p => p.userId === auth.currentUser?.uid)?.completedLessons?.includes(lesson.id);
            const isLocked = index > 0 && !isInstructor && !studentsProgress.find(p => p.userId === auth.currentUser?.uid)?.completedLessons?.includes(lessons[index-1]?.id);
            return (
              <div key={lesson.id} onClick={() => !isLocked && navigate(`/estudio/clase/${lesson.id}`)} className={`bg-white p-6 rounded-[35px] border-2 transition-all flex items-center gap-5 relative ${isLocked ? 'opacity-50 grayscale' : 'active:scale-95 cursor-pointer border-white shadow-sm'}`}>
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border-2 ${isLocked ? 'bg-slate-100 text-slate-300' : isCompleted ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-brand-50 text-brand-600'}`}>
                  {isLocked ? <Lock size={20}/> : isCompleted ? <CheckCircle size={22} /> : <span className="font-black text-lg">{lesson.order}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-black text-slate-800 text-sm uppercase truncate mb-1">{lesson.title}</h4>
                  <div className="flex items-center gap-3"><span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1"><Play size={10} fill="currentColor"/> Video</span></div>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  {!isLocked && <ArrowRight size={20} className="text-slate-100" />}
                </div>
              </div>
            );
          })}
        ...
        /* VISTA 2: EL OJO DEL PROFESOR (ESTADÍSTICAS) */
        <div className="px-6 space-y-6 animate-slide-up">
          ...
          <div className="space-y-3">
            {studentsProgress.map((p) => {
              const perc = (p.completedLessons?.length / lessons.length) * 100;
              return (
                <div key={p.id} onClick={() => setSelectedStudent(p)} className="bg-white p-5 rounded-[30px] border-2 border-white shadow-sm flex items-center gap-4 active:scale-95 transition-all cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       <div className="h-1 flex-1 bg-slate-50 rounded-full overflow-hidden border border-slate-100"><div className="h-full bg-brand-500" style={{width: `${perc}%`}}></div></div>
                       <span className="text-[8px] font-black text-slate-400">{p.completedLessons?.length || 0}/{lessons.length}</span>
                    </div>
                  </div>
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-[200] bg-slate-900/90 backdrop-blur-md flex items-end justify-center">
          <div className="bg-white w-full max-w-md rounded-t-[50px] animate-slide-up max-h-[85vh] flex flex-col relative">
             ...
             <div className="p-10 border-b border-slate-50 flex flex-col items-center">
                ...
                <h3 className="font-black text-slate-900 uppercase text-lg">{selectedStudent.userName}</h3>
                <p className="text-[10px] font-black text-brand-600 uppercase tracking-[0.2em] mt-1">Informe de Seguimiento</p>
             </div>

             <div className="flex-1 overflow-y-auto p-8 space-y-4 no-scrollbar">
                {lessons.map((l, idx) => {
                  const score = selectedStudent.grades?.[l.id];
                  const details = selectedStudent.details?.[l.id];
                  return (
                    <div key={l.id} className="bg-slate-50 p-6 rounded-[35px] border border-slate-100">
                       <div className="flex justify-between items-start mb-4">
                          ...
                          <div className={`px-4 py-2 rounded-xl text-[10px] font-black ${score >= 7 ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
                             {score ? `${score}/10` : 'Pendiente'}
                          </div>
                       </div>
                       
                       {/* DETALLE DE RESPUESTAS (Fase 4) */}
                       {details && (
                         <div className="space-y-2 mt-4 pt-4 border-t border-slate-200/50">
                            {l.quiz.questions.map((q, qIdx) => {
                              const isCorrect = details[qIdx] === q.correctAnswer;
                              return (
                                <div key={qIdx} className="flex items-center gap-2">
                                   {isCorrect ? <CheckCircle size={12} className="text-emerald-500 shrink-0"/> : <X size={12} className="text-rose-500 shrink-0"/>}
                                   <p className={`text-[9px] font-bold uppercase ${isCorrect ? 'text-slate-400' : 'text-rose-600'}`}>P{qIdx+1}: {q.options[details[qIdx]] || 'N/A'}</p>
                                </div>
                              );
                            })}
                         </div>
