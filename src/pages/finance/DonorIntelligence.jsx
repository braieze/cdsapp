...
export default function DonorIntelligence({ movements = [], setCustomAlert, selectedMonth, selectedYear }) {
  ...
  const [subView, setSubView] = useState('diezmos'); // diezmos o ofrendas
  const [eventFilter, setEventFilter] = useState(''); // Filtro por culto específico
  const [copied, setCopied] = useState(false);

  // ✅ 1. PROCESAMIENTO INTELIGENTE DE DATOS (CORREGIDO Y RESILIENTE)
  const donorsData = useMemo(() => {
    ...
    movements
      .filter(m => Number(m.total) > 0) // Solo ingresos con monto válido
      .forEach(m => {
        const mDate = m.date?.seconds ? new Date(m.date.seconds * 1000) : new Date();
        
        // Cargar eventos únicos para el filtro de cultos (Limpieza de etiquetas)
        ...
        const isSelectedMonth = mDate.getMonth() === Number(selectedMonth);
        const isSelectedYear = mDate.getFullYear() === Number(selectedYear);
        
        if (filter === 'week') passFilter = mDate >= startOfWeek;
        if (filter === 'month') passFilter = isSelectedMonth && isSelectedYear;
        if (filter === 'year') passFilter = isSelectedYear;
        if (filter === 'all') passFilter = true;

        // ✅ CORRECCIÓN: Detección inteligente de tipo para registros antiguos
        ...
        if (passFilter && matchesType && matchesEvent) {
          const name = m.fullName || m.concept?.replace("Transferencia: ", "").replace("Diezmo: ", "") || "Dador Anónimo";
  ...
  return (
    <div className="space-y-6 pt-4 pb-24 animate-fade-in text-left">
      ...
      <section className="px-2 grid grid-cols-2 gap-3">
        <div className="bg-slate-900/40 border border-white/5 p-5 rounded-[35px] backdrop-blur-xl shadow-2xl">
      ...
      <header className="space-y-4 px-2">
        ...
        <div className="flex bg-slate-900/50 p-1 rounded-2xl border border-white/5 shadow-inner">
            {['diezmos', 'ofrendas'].map((v) => (
                <button key={v} onClick={() => setSubView(v)} className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${subView === v ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:text-slate-300'}`}>
                    {v}
                </button>
            ))}
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex bg-slate-900/50 p-1.5 rounded-2xl border border-white/5 overflow-x-auto scrollbar-hide">
                ...
                <button key={f.id} onClick={() => setFilter(f.id)} className={`flex-1 px-3 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${filter === f.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500'}`}>{f.label}</button>
                ))}
            </div>
            {/* SELECTOR DE CULTO */}
            <div className="relative">
      ...
      <div className="space-y-4 px-1">
              ...
              <div className="space-y-3 pt-4 border-t border-white/5 relative z-10">
                ...
                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {d.prayers.length > 0 ? d.prayers.map((p, idx) => (
                    <div key={idx} className="bg-slate-950/50 p-4 rounded-[28px] border border-white/5 group/prayer transition-all hover:border-blue-500/30 text-left shadow-inner">
        ...
        {donorsData.list.length === 0 && (
          <div className="py-24 text-center">
            ...
            <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.4em] text-center">Sin registros hallados para {subView}</p>
          </div>
        )}
      </div>
    </div>
