...
export default function Profile() {
  ...
  // ✅ Sincronizamos con 'area' que es lo que usa el resto de la app
  const [formData, setFormData] = useState({
    phone: '', address: '', birthday: '', dni: '',
    bloodType: '', emergencyName: '', emergencyPhone: '', area: ''
  });
      ...
      if (currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          setFormData({
            ...
            area: data.area || 'ninguna' // Usamos area
          });
        }
      }
      setLoading(false);
  ...
  const handleSave = async () => {
    try {
      // ✅ Guardamos 'area' para que el Directorio lo vea
  ...
  const downloadPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(credentialRef.current, { scale: 4, useCORS: true, backgroundColor: "#ffffff" });
    ...
    <div className="pb-24 bg-slate-50 min-h-screen relative animate-fade-in font-outfit">
      ...
      <div className="text-center px-6 mb-8">
        ...
        <div className="flex justify-center gap-2 mt-3">
          ...
          <span className="px-3 py-1 rounded-lg bg-brand-50 text-brand-700 text-[10px] font-black uppercase tracking-widest border border-brand-100 flex items-center gap-1.5">
            <Briefcase size={12} className="text-brand-500" /> {formData.area}
          </span>
        </div>
      </div>
      ...
      {!isEditing && (
        <div className="px-6 mb-8">
          <button onClick={() => setShowCredential(true)} className="w-full bg-slate-900 text-white p-4 rounded-2xl flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-400 uppercase ml-2">WhatsApp</label>
                <input disabled={!isEditing} type="tel" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="w-full bg-slate-50 p-3 rounded-xl text-sm font-bold border border-transparent focus:border-brand-500 outline-none disabled:text-slate-600 transition-all" placeholder="11..." />
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-400 uppercase ml-2">Área / Ministerio</label>
                <input disabled={!isEditing} value={formData.area} onChange={e => setFormData({ ...formData, area: e.target.value.toLowerCase() })} className="w-full bg-slate-50 p-3 rounded-xl text-sm font-bold border border-transparent focus:border-brand-500 outline-none disabled:text-slate-600 transition-all" placeholder="recepcion, etc" />
              </div>
            </div>
            ...
            {/* CUMPLEAÑOS Y DIRECCIÓN */}
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-400 uppercase ml-2">Cumpleaños</label>
                <input disabled={!isEditing} type="date" value={formData.birthday} onChange={e => setFormData({ ...formData, birthday: e.target.value })} className="w-full bg-slate-50 p-3 rounded-xl text-sm font-bold border border-transparent focus:border-brand-500 outline-none disabled:text-slate-600 transition-all" />
              </div>
              <div className="space-y-1">
            ...
            {(isEditing || (formData.emergencyName || formData.emergencyPhone)) && (
              <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100">
                ...
                <div className="space-y-2">
                  <input disabled={!isEditing} value={formData.emergencyName} onChange={e => setFormData({ ...formData, emergencyName: e.target.value })} className="w-full bg-white p-3 rounded-xl text-xs font-bold border border-rose-200 outline-none" placeholder="Nombre contacto" />
                  <input disabled={!isEditing} value={formData.emergencyPhone} onChange={e => setFormData({ ...formData, emergencyPhone: e.target.value })} className="w-full bg-white p-3 rounded-xl text-xs font-bold border border-rose-200 outline-none" placeholder="Teléfono contacto" />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-[500] bg-slate-900/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-fade-in">
          ...
          <div ref={credentialRef} className="w-full max-w-[320px] bg-white rounded-2xl overflow-hidden shadow-2xl relative flex flex-col">
            <div className="p-8 pb-10 flex flex-col items-center text-center bg-white flex-1">
              ...
              <h3 className="text-2xl font-black text-slate-800 uppercase leading-tight mb-1">{currentUser?.displayName}</h3>
              <p className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">{userRole}</p>
              {formData.area && <span className="px-3 py-0.5 bg-brand-50 text-brand-700 text-[9px] font-black uppercase tracking-widest rounded-full border border-brand-100 inline-block mt-1">{formData.area}</span>}
          ...
          <button onClick={downloadPDF} disabled={isGeneratingPDF} className="w-full max-w-[320px] bg-brand-600 text-white py-4 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-3 shadow-lg mt-6">
            {isGeneratingPDF ? <Loader2 className="animate-spin" size={18} /> : <><Download size={18} /> Descargar PDF</>}
          </button>
        </div>
      )}
    </div>
