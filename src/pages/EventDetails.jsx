...
import { doc, getDoc, updateDoc, collection, getDocs, deleteDoc, serverTimestamp, arrayUnion, arrayRemove } from 'firebase/firestore'; 
import { 
  X, Calendar, Clock, Save, Trash2, Plus, Users, 
  CheckCircle, Download, Loader2, Search, HelpCircle,
  AlertCircle, Check, ExternalLink, ArrowRight, UserPlus, UserMinus
} from 'lucide-react';
...
export default function EventDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Escuchar cambios en el evento en tiempo real (vital para inscripciones de ayuno)
  ...
  // ✅ 1. LÓGICA DE AYUNO: ANOTARSE / DARSE DE BAJA
  const toggleFastingSignup = async (dateStr) => {
    if (!currentUser) return;
    const eventRef = doc(db, 'events', id);
    const fastingData = event.fastingSignups || {};
    const daySignups = fastingData[dateStr] || [];
    ...
    try {
      ...
      const updatedFast = { ...fastingData, [dateStr]: isAlreadySignedUp 
      ...
      setToast({ message: isAlreadySignedUp ? "Te has dado de baja" : "¡Anotado con éxito!", type: "success" });
    } catch (e) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`CDS-${event.title}.pdf`);
      setToast({ message: "Reporte descargado", type: "success" });
  ...
  // Generar rango de días para Ayuno
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-fade-in overflow-hidden font-outfit">
        ...
        <div className="max-w-xl mx-auto space-y-8">
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-2">
                <div className="flex items-center gap-2 bg-slate-50 px-5 py-3 rounded-2xl text-[11px] font-black text-slate-700 border border-slate-100 uppercase tracking-widest shadow-sm">
                    ...
                    {(!event.endDate || event.endDate === event.date) && format(parseISO(event.date), " 'de' MMMM", { locale: es })}
                </div>
                <div className="flex items-center gap-2 bg-slate-50 px-5 py-3 rounded-2xl text-[11px] font-black text-slate-700 border border-slate-100 uppercase tracking-widest shadow-sm">
            ...
            {/* CONTENIDO ESPECIAL: AYUNO CONGREGACIONAL */}
            {event.type === 'ayuno' ? (
              <div className="space-y-6">
                <div className="bg-rose-50 p-6 rounded-[35px] border-2 border-rose-100">
                  ...
                  <p className="text-rose-900/60 text-[11px] font-bold leading-relaxed uppercase">
                    Selecciona el día que te comprometes a ayunar. ¡Podés elegir varios!
                  </p>
                </div>
                ...
                <div className="grid gap-4">
                  {fastingDays.map((day) => {
                    ...
                    return (
                      <div key={dateStr} className={`p-5 rounded-[30px] border-2 transition-all flex flex-col gap-4 ${amISignedUp ? 'bg-white border-rose-500 shadow-xl scale-[1.02]' : 'bg-slate-50 border-slate-100'}`}>
                        <div className="flex justify-between items-center">
                          ...
                          <button 
                            onClick={() => toggleFastingSignup(dateStr)}
                            className={`p-4 rounded-2xl shadow-lg transition-all active:scale-90 ${amISignedUp ? 'bg-rose-500 text-white' : 'bg-white text-slate-400'}`}
                          >
                            {amISignedUp ? <UserMinus size={20}/> : <UserPlus size={20}/>}
                          </button>
                        </div>

                        {/* LISTA DE PERSONAS ANOTADAS */}
                        <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                          <p className="w-full text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Anotados ({daySignups.length})</p>
                          {daySignups.length > 0 ? daySignups.map((name, i) => (
                            <span key={i} className="px-3 py-1.5 bg-white border border-slate-100 rounded-xl text-[10px] font-black text-slate-600 uppercase shadow-sm">
                              {name.split(' ')[0]}
                            </span>
                          )) : <p className="text-[9px] italic text-slate-300 font-bold">¡Sé el primero en anotarte!</p>}
                        </div>
                      </div>
                    );
                  })}
              ...
              /* CONTENIDO NORMAL: ESTRUCTURA POR ROLES */
              <div className="space-y-6">
                                    ...
                                    <div key={role.key}>
                                        ...
                                        <div className="flex flex-wrap gap-2.5">
                                            {assigned.length > 0 ? assigned.map((p, i) => (
                                                <div key={i} className={`inline-flex items-center gap-3 px-4 py-2.5 rounded-2xl text-xs font-black uppercase border transition-all ${isEditing ? 'bg-brand-600 text-white border-brand-400' : 'bg-slate-50 border-slate-100 text-slate-700'}`}>
      ...
      {/* SELECTOR DE PERSONA (BUSCADOR FIJO) */}
      {isSelectorOpen && (
              ...
              <div className="bg-white w-full max-w-md rounded-t-[50px] h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
                  <div className="p-8 border-b flex justify-between items-center bg-white shrink-0">
                      <div>
                        ...
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Busca en la congregación</p>
                      </div>
                  ...
                  <div className="p-5 bg-slate-50 shrink-0">
                      <div className="bg-white border-2 border-slate-100 rounded-2xl px-5 py-4 flex items-center gap-3 shadow-inner">
                          <Search size={18} className="text-slate-400"/><input autoFocus type="text" placeholder="Escribir nombre..." className="w-full text-sm font-bold outline-none bg-transparent" value={personSearchTerm} onChange={e => setPersonSearchTerm(e.target.value)}/>
                      </div>
                  </div>
                  <div className="flex-1 overflow-y-auto p-5 space-y-3 no-scrollbar pb-10">
                      {users.filter(u => (u.displayName || '').toLowerCase().includes(personSearchTerm.toLowerCase())).map(u => {
                          ...
                          return (
                              ...
                              >
                                  <div className="w-14 h-14 rounded-2xl border-2 border-white shadow-md overflow-hidden bg-slate-100 shrink-0">
                                  ...
                                  <div className="flex-1 min-w-0">
                                      ...
                                      <p className={`text-[10px] font-bold uppercase mt-1 tracking-widest ${isAlreadyInThisRole ? 'text-white/60' : 'text-slate-400'}`}>{u.area || 'Miembro'}</p>
                                  </div>
