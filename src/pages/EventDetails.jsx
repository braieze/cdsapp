...
export default function EventDetails() {
  ...
  // ✅ ESTADÍSTICAS PARA EL "ESTADO GENERAL"
  ...
  // ✅ ESTRUCTURA CORREGIDA (Sincronizada con tu captura image_ec7653.png)
  const getStructure = () => {
    ...
    return [
      ...
      { section: 'OPERATIVO / UJERES', roles: [
        { key: 'puerta', label: 'Puerta', icon: Users, type: 'multi' },
        { key: 'porteria', label: 'Portero (cerrar iglesia)', icon: Lock, type: 'single' },
        { key: 'pasillo', label: 'Pasillo', icon: ArrowRight, type: 'multi' },
  ...
  return (
    <div className={`fixed inset-0 z-[100] bg-white flex flex-col animate-fade-in overflow-hidden font-outfit ${event.isCena ? 'border-t-[10px] border-rose-600' : ''}`}>
        ...
        <div className="max-w-xl mx-auto space-y-8">
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-4">
                <div className="flex items-center gap-2 bg-slate-50 px-5 py-3 rounded-2xl border border-slate-100 shadow-sm">
                   ...
                   <span className="text-[11px] font-black text-slate-700 uppercase tracking-tight">{format(parseISO(event.date), "d 'de' MMMM yyyy", { locale: es })}</span>
                </div>
                <div className="flex items-center gap-2 bg-slate-50 px-5 py-3 rounded-2xl border border-slate-100 shadow-sm">
                   <Clock size={14} className="text-brand-500"/>
                   <span className="text-[11px] font-black text-slate-700 uppercase tracking-tight">{event.time} hs</span>
                </div>
            </div>
      ...
      {/* ✅ BARRA "ESTADO GENERAL" - FIJA ABAJO */}
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-slate-900 rounded-t-[50px] shadow-2xl z-50 flex items-center justify-between animate-slide-up">
          ...
          <button onClick={() => navigate(-1)} className="bg-slate-800 text-white px-10 py-5 rounded-[22px] font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all">Entendido</button>
      </div>
