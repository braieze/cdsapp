...
import { X, Calendar, Clock, MapPin, Save, Trash2, Plus, ChevronDown, Users, CheckCircle, Edit3, CheckSquare, Search, Download, HelpCircle, Loader2, UserCheck } from 'lucide-react';
import { EVENT_TYPES } from '../utils/eventTypes';
import { format, eachDayOfInterval } from 'date-fns';
import { es } from 'date-fns/locale';
import html2canvas from 'html2canvas'; // ✅ Requerido para PDF
import jsPDF from 'jspdf'; // ✅ Requerido para PDF

export default function EventDetails() {
  const { id } = useParams();
  const navigate = useNavigate();
  const reportRef = useRef(); // ✅ Referencia para la captura del PDF
  ...
  // ✅ FUNCIONALIDAD: DESCARGA DE PDF DIRECTA (Estilo Credencial)
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-fade-in overflow-hidden font-outfit">
      ...
      {/* CUERPO - Capturado por la referencia PDF */}
      ...
      {/* ✅ MODAL SELECTOR CON FILTRO DINÁMICO Y BLINDAJE REAL */}
      {isSelectorOpen && (
              ...
              <div className="bg-white w-full sm:max-w-sm rounded-t-[40px] sm:rounded-[40px] max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-white">
                      {users.filter(u => {
                          ...
                          // FILTRO DINÁMICO: Si el buscador está vacío, mostramos solo los del área para agilizar.
                          // Pero permitimos ver a todos si el usuario busca un nombre específico.
                          ...
                          return (
                              <button 
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-[28px] border-2 transition-all text-left relative ${isAlreadySelectedInThisRole ? 'bg-brand-50 border-brand-500' : isTaken ? 'bg-slate-50 border-transparent opacity-40 grayscale cursor-not-allowed' : 'bg-white border-slate-50 hover:border-brand-100 active:scale-95 shadow-sm'}`}
                              >
      ...
      {toast && (
        <div className="fixed bottom-24 left-6 right-6 z-[400] animate-slide-up">
          <div className={`flex items-center gap-4 px-8 py-5 rounded-[30px] shadow-2xl border-2 ${toast.type === 'success' ? 'bg-emerald-600 text-white border-emerald-400' : 'bg-slate-900 text-white border-slate-700'}`}>
            {toast.type === 'success' ? <CheckCircle size={24}/> : <Info size={24}/>}
