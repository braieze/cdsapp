...
import { X, Calendar, Clock, MapPin, Save, Trash2, Plus, ChevronDown, Users, CheckCircle, Edit3, CheckSquare, Search, Printer, HelpCircle, Loader2, UserCheck } from 'lucide-react';
...
export default function EventDetails() {
  ...
  // ✅ FUNCIONALIDAD DE BLINDAJE: ¿ESTÁ YA SELECCIONADO EN ESTE EVENTO?
  ...
  const handleSaveAssignments = async () => {
    ...
    try {
      ...
      setIsEditing(false);
    } catch (error) { alert("Error al guardar"); } finally { setIsSaving(false); }
  };
    ...
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-fade-in overflow-hidden font-outfit">
      ...
      {isEditing && (
          <div className="p-4 bg-white border-t border-slate-100 absolute bottom-0 w-full shadow-2xl flex gap-3 z-50 animate-slide-up">
              ...
              <button onClick={handleSaveAssignments} disabled={isSaving} className="flex-1 bg-slate-900 text-white font-black py-4 rounded-2xl shadow-xl flex items-center justify-center gap-2">
      ...
      {/* ✅ MODAL SELECTOR CON FILTRO Y BLINDAJE REAL */}
      {isSelectorOpen && (
              ...
              <div className="bg-white w-full sm:max-w-sm rounded-t-[40px] sm:rounded-[40px] max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-white">
                      {users.filter(u => {
                          ...
                          // FILTRO POR MINISTERIO (Imagen 8): Mostramos primero los del área o si no hay búsqueda mostramos los sugeridos
                          const userArea = (u.area || u.ministerio || '').toLowerCase();
                          const roleLabel = (activeRoleConfig?.label || '').toLowerCase();
                          const isPastoral = roleLabel.includes('predica') || roleLabel.includes('oración') || roleLabel.includes('ofrenda');
                          const isLeader = u.role === 'pastor' || u.role === 'lider';
                          
                          const areaMatches = (isPastoral && isLeader) || (userArea !== 'ninguna' && roleLabel.includes(userArea));
                          
                          // Si no hay búsqueda, priorizamos los del área. Si hay búsqueda, mostramos todos los que coincidan.
                          return personSearchTerm ? matchesSearch : (matchesSearch && areaMatches);
                      }).map(u => {
                          ...
                          return (
                              <button 
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-[28px] border-2 transition-all text-left relative ${isAlreadySelectedInThisRole ? 'bg-brand-50 border-brand-500' : isTaken ? 'bg-slate-50 border-transparent opacity-40 grayscale cursor-not-allowed' : 'bg-white border-slate-50 hover:border-brand-200 active:scale-95'}`}
                              >
                                  <div className="w-14 h-14 rounded-[22px] bg-slate-100 overflow-hidden border-2 border-white shadow-sm flex items-center justify-center font-black text-slate-400 relative">
                                      ...
                                      {isAlreadySelectedInThisRole && <div className="absolute inset-0 bg-brand-500/20 flex items-center justify-center"><UserCheck size={20} className="text-brand-600"/></div>}
                                  </div>
                                  <div className="flex-1">
                                      <p className="font-black text-slate-800 text-sm uppercase tracking-tight">{u.displayName}</p>
                                      {isTaken && !isAlreadySelectedInThisRole ? (
                                          <p className="text-[8px] text-rose-500 font-black uppercase tracking-widest animate-pulse mt-0.5">Ya tiene un servicio asignado</p>
                                      ) : (
                                          <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-0.5">{u.area || 'Miembro'}</p>
                                      )}
                                  </div>
