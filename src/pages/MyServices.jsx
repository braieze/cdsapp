...
import { format, isSameMonth, isAfter, startOfDay } from 'date-fns';
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                ...
                // ✅ FIX BUG DE CONFIRMACIÓN: Sincronizamos la medianoche de hoy
                ...
                // Cálculo de alertas exactas para coincidir con BottomNavigation
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-slate-900/40 backdrop-blur-sm flex items-end justify-center p-0 sm:p-4 animate-fade-in">
      <div className="bg-white w-full max-w-sm rounded-t-[32px] sm:rounded-[32px] p-6 sm:p-8 shadow-2xl animate-slide-up text-center border border-slate-100">
        ...
        <h4 className="font-bold text-slate-900 text-xl mb-2">{confirmAction.status === 'confirmed' ? 'Confirmar Asistencia' : 'Informar Baja'}</h4>
      ...
      <div className="bg-white w-full sm:max-w-md rounded-t-[32px] sm:rounded-[32px] max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-slide-up border border-slate-100" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white flex-shrink-0">
          <div className="text-left">
            <h3 className="font-bold text-slate-900 text-lg leading-none truncate pr-4">{event.title}</h3>
            <p className="text-[11px] font-semibold text-slate-500 mt-1 capitalize">{format(new Date(event.date + 'T00:00:00'), 'EEEE d MMMM yyyy', {locale: es})}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-slate-50 rounded-full text-slate-500 active:scale-90 transition-transform"><X size={18}/></button>
        </div>
