...
export default function LessonView() {
  ...
  // 🛠️ ARREGLO PDF (NUCLEAR): Transforma el link de Drive en un objeto incrustable real
  const getEmbedPdfUrl = (url) => {
    if (!url) return null;
    if (url.includes('drive.google.com')) {
      // Reemplazamos /view o /edit por /preview para que Drive suelte el PDF real
  ...
  return (
    <div className="bg-slate-50 min-h-screen animate-fade-in font-outfit text-left flex flex-col relative">
      
      {/* 📌 VIDEO FIJO (STICKY) - No se mueve al hacer scroll */}
      <div className="sticky top-0 z-50 bg-slate-50 shadow-md">
        <div className="px-6 pt-4 mb-2 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] font-black text-brand-600 uppercase truncate max-w-[180px]">{study?.title}</p>
          </div>
        </div>
      ...
      {/* CUERPO DESLIZABLE */}
      <div className="flex-1 px-8 space-y-10 pt-6 pb-40 overflow-y-visible">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
               {lesson.gallery.map((img, idx) => (
                 <img key={idx} src={img} className="h-48 rounded-[30px] border-4 border-white shadow-lg shrink-0 object-cover" onClick={() => window.open(img, '_blank')} />
               ))}
            </div>
          </div>
        )}

        {/* 📑 SECCIÓN PDF: ARREGLADA */}
        {lesson.pdfUrl && (
          <div className="space-y-4">
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] px-2">Guía de Estudio</p>
            <div className="w-full h-[500px] bg-slate-200 rounded-[35px] overflow-hidden border-4 border-white shadow-inner relative">
               {/* Usamos directamente el link de preview de Drive sin el visor de google encima */}
               <iframe 
            ...
            <a href={lesson.pdfUrl} target="_blank" rel="noreferrer" className="w-full bg-white p-5 rounded-2xl flex items-center justify-between border border-slate-100 shadow-sm">
               <div className="flex items-center gap-3"><FileText size={20} className="text-brand-600"/><span className="text-[11px] font-black uppercase">Descargar PDF Original</span></div>
        ...
        {lesson.externalLink?.url && (
          <a href={lesson.externalLink.url} target="_blank" rel="noreferrer" className="bg-slate-900 p-6 rounded-[35px] text-white flex items-center justify-between group active:scale-95 transition-all shadow-lg">
             <div className="flex items-center gap-4">
                <div className="p-3 bg-white/10 rounded-2xl"><LinkIcon size={20} className="text-brand-400"/></div>
                <div className="text-left min-w-0">
                   <p className="text-[8px] font-black uppercase tracking-widest text-brand-400 leading-none mb-1">Link Externo</p>
        ...
        <div className="pt-4 text-center">
           {!quizStarted && !quizResult && (
             <div className="bg-white p-10 rounded-[45px] border-2 border-dashed border-slate-200 space-y-6 shadow-sm">
                ...
                <h4 className="font-black text-slate-900 uppercase tracking-tighter text-xl">¿Hacer el examen?</h4>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest px-4 leading-relaxed">Debes aprobar para completar la clase</p>
                <button onClick={() => setQuizStarted(true)} className="w-full bg-slate-900 text-white font-black py-5 rounded-3xl shadow-xl uppercase text-[11px] tracking-widest active:scale-95 transition-all">Comenzar Ahora</button>
             </div>
           )}
           
           {quizResult?.alreadyDone && (
              <div className="bg-emerald-50 p-10 rounded-[45px] border-2 border-emerald-100 space-y-4 shadow-sm">
                 <div className="w-20 h-20 bg-emerald-500 text-white rounded-[30px] flex items-center justify-center mx-auto"><CheckCircle size={40} /></div>
                 <h3 className="text-2xl font-black text-emerald-900 uppercase tracking-tighter">Completada</h3>
                 <p className="text-xs text-emerald-600 font-bold uppercase">Nota: {quizResult.score} / 10</p>
      ...
      {/* 🎯 MODAL DE EXAMEN (Punto 2) - Se superpone a todo */}
      {(quizStarted && !quizResult) && (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex items-center justify-center p-6 overflow-y-auto">
          <div className="w-full max-w-xl animate-scale-in bg-white p-8 rounded-[45px] border-2 border-white shadow-2xl space-y-8">
      ...
      {(quizResult && !quizResult.alreadyDone) && (
        <div className="fixed inset-0 z-[110] bg-slate-900/98 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md p-10 rounded-[50px] border-2 border-white shadow-2xl text-center space-y-8 animate-scale-in">
                ...
                {lesson.quiz.questions.map((q, idx) => (
                    <div key={idx} className={`p-4 rounded-2xl border-2 ${answers[idx] === q.correctAnswer ? 'border-emerald-50 bg-emerald-50/50' : 'border-rose-50 bg-rose-50/50'}`}>
                      ...
                      <div className="flex justify-between items-center">
                          <p className="text-[9px] font-bold text-slate-400 uppercase">Tú: <span className={answers[idx] === q.correctAnswer ? 'text-emerald-600' : 'text-rose-600'}>{q.options[answers[idx]] || 'N/A'}</span></p>
