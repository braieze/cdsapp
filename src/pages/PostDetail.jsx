...
import { 
  ...
  ChevronLeft, BookOpen, HandHeart, Lock
} from 'lucide-react';
...
export default function PostDetail() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      setAuthChecked(true); // Ya sabemos si es invitado o usuario
    });
    return () => unsubAuth();
  }, []);
  ...
  useEffect(() => {
    // Esperamos a saber el estado de autenticación antes de cargar el post
    ...
    const unsubPost = onSnapshot(doc(db, 'posts', postId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        // 🛡️ GUARDIA DE SEGURIDAD: Bloquea si es privado y no hay sesión
  ...
  const handleReaction = async (emoji) => {
    if (!authUser) return navigate('/login'); // Invitados no pueden reaccionar
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-[#F8F9FE] flex flex-col animate-fade-in overflow-hidden font-sans text-left">
      ...
      <footer className="fixed bottom-0 w-full left-1/2 -translate-x-1/2 max-w-md bg-[#F8F9FE]/90 backdrop-blur-md pb-6 pt-3 px-5 z-50">
        {!authUser ? (
          <button onClick={() => navigate('/login')} className="w-full bg-blue-600 text-white rounded-full py-3.5 font-bold text-[13px] flex items-center justify-center gap-2 shadow-sm">
