...
export default function AppsHub() {
  const { dbUser } = useOutletContext(); // ✅ Obtenemos el rol en tiempo real
  ...
  const allApps = [
    ...
    // 🔐 Solo Pastor y Tesorero ven Tesorería
    { id: 'tesoreria_admin', name: 'Tesorería', icon: Wallet, color: 'text-slate-900', bg: 'bg-slate-200', path: '/tesoreria', roles: ['pastor', 'tesorero'] },
    
    { id: 'historial', name: 'Historial', icon: History, color: 'text-indigo-600', bg: 'bg-indigo-50', path: '/historial', roles: ['todos'] },
  ...
  const visibleApps = allApps.filter(app => 
    ...
    app.roles.includes(dbUser?.role?.toLowerCase()) ||
    app.roles.includes(dbUser?.area?.toLowerCase())
  );
    ...
    <div className="pb-32 pt-4 px-4 animate-fade-in bg-slate-50 min-h-screen font-outfit text-left">
      ...
      {['pastor', 'lider'].includes(dbUser?.role) && (
          <div className="mt-8 flex items-center justify-center gap-2 opacity-30">
              ...
              <span className="text-[8px] font-black uppercase tracking-[0.3em] text-slate-400">Sesión de Administrador Protegida</span>
          </div>
      )}
    </div>
  );
