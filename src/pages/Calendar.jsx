...
import {
  ...
  Church, Users, Music, Eraser, Wrench, Flame, Lock
} from 'lucide-react';
import { 
  format, addMonths, subMonths, isSameMonth, startOfMonth, 
  endOfMonth, startOfWeek, endOfWeek, eachDayOfInterval, 
  isSameDay, isWithinInterval, parseISO, isAfter, startOfDay 
} from 'date-fns';
import { es } from 'date-fns/locale';
import imageCompression from 'browser-image-compression';

// ✅ CONFIGURACIÓN DE TIPOS OPERATIVOS (Punto 1)
...
export default function CalendarPage() {
  ...
  const [editingId, setEditingId] = useState(null); // ✅ Nuevo para Edición
  ...
  // --- 3. NOTIFICACIONES (Igualadas al TopBar para iPhone/Windows) ---
  ...
  // --- 4. FILTRADO INTELIGENTE (Punto 2 y 3) ---
  ...
  }, [events, filterType, timeFilter, currentDate, userRole, currentUser]);

  // --- 5. CREACIÓN Y EDICIÓN (Punto 2) ---
    ...
    if (filteredEvents.length === 0) return (
      <div className="py-24 text-center opacity-30 flex flex-col items-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-center leading-loose">
          {filterType === 'mine' ? 'Sin tareas asignadas\nen este periodo' : 'Sin actividades'}
        </p>
      </div>
    );

    return (
      <div className="space-y-4 animate-fade-in px-4 text-left pb-24">
          {filteredEvents.map(event => {
            ...
            return (
              <div key={event.id} onClick={() => canAccess ? navigate(`/calendario/${event.id}`) : setToast({message: "Acceso Privado", type: "error"})}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    ...
                    {['pastor', 'lider'].includes(userRole) && (
                      <div className="flex gap-1">
                        <button onClick={(e) => { e.stopPropagation(); setEditingId(event.id); setNewEvent(event); setIsModalOpen(true); }} className="p-1.5 text-slate-300 hover:text-brand-600"><Edit3 size={16}/></button>
                        <button onClick={(e) => { e.stopPropagation(); setActionConfirm({ type: 'delete', id: event.id, title: '¿Borrar?', message: 'Esta acción no se puede deshacer.' }); }} className="p-1.5 text-slate-200 hover:text-rose-500"><Trash2 size={16}/></button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><Clock size={12} className="text-brand-500"/> {event.time} hs</span>
                    {event.isCena && <span className="text-[9px] font-black text-rose-500 uppercase flex items-center gap-1">🍷 Cena</span>}
                  </div>
                </div>
              </div>
            );
  ...
  return (
    <div className="pb-36 pt-4 bg-slate-50 min-h-screen animate-fade-in relative font-outfit">
      ...
      {/* AVISO BORRADORES (Pastor) */}
      {['pastor', 'lider'].includes(userRole) && viewMode === 'list' && events.some(e => !e.published && isSameMonth(new Date(e.date + 'T00:00:00'), currentDate)) && (
          <div className="mx-6 bg-amber-500 p-6 rounded-[35px] mb-8 flex items-center justify-between shadow-xl shadow-amber-200/30">
      ...
      {/* MODAL PLANIFICAR/EDITAR (Punto 2) */}
      {isModalOpen && (
            ...
            <div className="bg-white w-full max-w-sm rounded-[45px] p-8 shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar relative animate-slide-up text-left">
                ...
                <div className="space-y-6">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase ml-4">Título</label>
                      <input placeholder="Nombre..." className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[24px] font-black text-slate-800 outline-none focus:border-brand-500 uppercase text-sm" value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} />
                    </div>
