            ...
            if (['pastor', 'lider'].includes(userRole) && data.confirmations) {
                ...
                if (declines.length > 0) {
                  asgs.push({ 
                    id: `dec-group-${d.id}`, 
                    type: 'alert', 
                    title: 'Bajas en el Equipo', 
                    subtitle: `Hay ${declines.length} servidores que no asisten a: ${data.title}`, 
                    timestamp: Date.now(), 
        ...
        <div className="fixed inset-0 z-[110] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-5 font-outfit animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-8 shadow-2xl animate-scale-in flex flex-col gap-6">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Lanzar Aviso Push</h3>
                <p className="text-[10px] font-black text-brand-600 uppercase tracking-widest">Llega a todos los celulares</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <textarea 
                ...
                className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-semibold text-sm h-28 resize-none outline-none focus:border-brand-500 transition-all"
                value={pushData.body} onChange={e => setPushData({...pushData, body: e.target.value})}
              />
              <div className="p-4 bg-indigo-50 rounded-2xl border-2 border-indigo-100 space-y-2">
                <p className="text-[9px] font-black text-indigo-600 uppercase flex items-center gap-2"><LinkIcon size={12}/> Link de Oración / Meet (Opcional)</p>
                <input 
                  placeholder="https://meet.google.com/..." 
                  className="w-full p-2 bg-white border border-indigo-200 rounded-xl font-bold text-[11px] outline-none text-indigo-700"
